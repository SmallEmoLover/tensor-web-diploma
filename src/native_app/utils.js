 ...
 */
export async function makeApiRequest(method, params = {}) {
    let urlParams = '';
    Object.keys(params).forEach((key) => {
        urlParams += `&${key}=${params[key]}`;
    });
    const response = await fetch(`http://ws.audioscrobbler.com/2.0/?method=${method}${urlParams}&api_key=${API_KEY}&format=json`);
 ...
 */
export function getItterableArrayByClassName(className) {
 ...
 * @param height
 */
export function generateImageUrl(width, height) {
    if (height === undefined) {
        height = width;
    }
