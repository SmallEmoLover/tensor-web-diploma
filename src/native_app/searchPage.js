 ...
 */
export default function showSearchPage(app, query) {
 ...
 * @param query - запрос для поиска
 */
function fillArtists(app, query) {
...
function fillAlbums(app, query) {
    ...
    return makeApiRequest('album.search', { album: query }).then((response) => {
        ...
        albums.slice(0, CARD_AMOUNT)
            .forEach((album) => {
            albumsList.append(makeAlbumCard(album));
        });
    }).catch(() => {
...
function fillTracks(app, query) {
    ...
    return makeApiRequest('track.search', { track: query }).then((response) => {
        ...
        tracks.slice(0, CARD_AMOUNT)
            .forEach((track) => {
            trackList.append(makeTrackRow(track));
        });
    }).catch(() => {
